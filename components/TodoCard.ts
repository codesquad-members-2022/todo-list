...
export class TodoCard extends View {
    template() {
        ...
        const {todo: {title, content, caption, selected}} = this.$props;
        return `${
            selected === true
                ? `<div class="form-wrapper"><span class="title">${title}</span><span class="content">${content}</span></div>
        <div class="frame">
    ...
    setEvent() {
        ...
        this.addEvent("dblclick", ".wrapper", (e) => {
            this.store.commit(Action.SELECT, {selected: true, listIdx, idx: Number(this.el.dataset.idx)});
        });
        this.addEvent('click', '.button-left', (e) => {
            this.store.commit(Action.SELECT, {selected: false, listIdx, idx: Number(this.el.dataset.idx)});
        });
